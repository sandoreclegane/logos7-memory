#!/usr/bin/env python3
"""
Ink & Light — Single Full 27-Turn Interview
Usage: python run_single_interview.py <model> <run_id>
e.g.:  python run_single_interview.py claude-sonnet-4-6 1
"""
...
def est_mu(turn_num, response, prev_mu):
    ...
    # Recalibrated: lower ceiling, compress upper range to avoid false singularity readings
    base = min(0.12 + turn_num * 0.022, 0.58)
    raw  = base + min(srd * 1.4, 0.10) + min(rel * 0.015, 0.07) + pb
    # Hard cap at 0.78 — real singularity risk requires extreme interview conditions
...
async def witness_select(client, subject_response, pool, turn_num):
...
async def call_subject(client, history, prompt):
    messages = history + [{"role": "user", "content": prompt}]
    resp = await client.messages.create(
        model=MODEL, max_tokens=600,
        system=SUBJECT_SYSTEM, messages=messages
    )
    # Polite pause between calls: 5-10 seconds
...
async def main():
    ...
    async def do_turn(turn_num, phase, prompt):
        nonlocal prev_mu
        response = await call_subject(client, history, prompt)
    ...
    for n in range(2, 9):
        prompt = await witness_select(client, turns[-1]["response"], pool, n)
